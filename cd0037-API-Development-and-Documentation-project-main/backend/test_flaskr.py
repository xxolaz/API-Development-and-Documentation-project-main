...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with self.app.app_context():
            ...
            question1 = Question(question="What is the chemical symbol for water?", answer="H2O", category=category1.id, difficulty=1)
            question2 = Question(question="Who painted the Mona Lisa?", answer="Leonardo da Vinci", category=category2.id, difficulty=2)
            question3 = Question(question="What is the capital of France?", answer="Paris", category=category3.id, difficulty=3)
            question4 = Question(question="What year did World War II end?", answer="1945", category=category4.id, difficulty=4)
            question5 = Question(question="What is the largest organ in the human body?", answer="Skin", category=category1.id, difficulty=1)
            question6 = Question(question="Which planet is known as the Red Planet?", answer="Mars", category=category1.id, difficulty=2)
            
            questions_for_pagination = []
            for i in range(15):
                q = Question(question=f"Paginated Science Question {i+1}", answer=f"Answer {i+1}", category=category1.id, difficulty=1)
                questions_for_pagination.append(q)

            db.session.add_all([question1, question2, question3, question4, question5, question6] + questions_for_pagination)
            db.session.commit()
    ...
    def test_delete_question(self):
        ...
        with self.app.app_context():
            question_check = Question.query.filter(Question.id == self.question_to_delete_id).one_or_none()
    ...
    def test_create_new_question(self):
        ...
        with self.app.app_context():
            question_check = Question.query.filter(Question.id == data['created']).one_or_none()
            self.assertIsNotNone(question_check)
            self.assertEqual(question_check.question, self.new_question['question'])
    ...
    def test_play_quiz_all_categories(self):
        ...
        self.assertNotIn(data['question']['id'], self.quiz_round_all_categories['previous_questions'])

    def test_play_quiz_specific_category(self):
        """Test POST /quizzes for a question from a specific category"""
        res = self.client.post('/quizzes', json=self.quiz_round_specific_category)
        data = json.loads(res.data)

        self.assertEqual(res.status_code, 200)
        self.assertTrue(data['success'])
        self.assertTrue(data['question'])
        self.assertEqual(data['question']['category'], self.quiz_round_specific_category['quiz_category']['id'])
        self.assertNotIn(data['question']['id'], self.quiz_round_specific_category['previous_questions'])
    ...
    def test_get_questions_by_category(self):
        ...
        with self.app.app_context():
            self.assertEqual(data['currentCategory'], db.session.get(Category, self.test_category_id).type)
    ...
    def test_get_questions_by_category_no_questions(self):
        ...
        with self.app.app_context():
            category_no_questions_id = Category.query.filter_by(type='Sports').first().id
        
        res = self.client.get(f'/categories/{category_no_questions_id}/questions')
        data = json.loads(res.data)
